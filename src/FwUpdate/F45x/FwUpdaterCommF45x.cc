...
FwUpdaterCommF45x::FwUpdaterCommF45x(const SerialConfig& config)
{
...
FwUpdaterCommF45x::~FwUpdaterCommF45x()
{
    try
    {
...
void FwUpdaterCommF45x::ReaderThreadLoop()
{
...
void FwUpdaterCommF45x::ConsumeScanned()
{
    _scan_index = _read_index.load();
}

char* FwUpdaterCommF45x::GetScanPtr() const
{
...
void FwUpdaterCommF45x::WriteBinary(const char* buf, size_t n_bytes)
{
#ifdef _WIN32
...
void FwUpdaterCommF45x::WaitForStr(const char* wait_str, std::chrono::milliseconds timeout)
{
...
void FwUpdaterCommF45x::StopReaderThread()
{
...
void FwUpdaterCommF45x::DumpSession(const char* filename)
{
