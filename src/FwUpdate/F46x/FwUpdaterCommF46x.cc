...
FwUpdaterCommF46x::FwUpdaterCommF46x(const SerialConfig& config)
{
...
FwUpdaterCommF46x::~FwUpdaterCommF46x()
{
    try
    {
...
void FwUpdaterCommF46x::ReaderThreadLoop()
{
...
void FwUpdaterCommF46x::ConsumeScanned()
{
    _scan_index = _read_index.load();
}

char* FwUpdaterCommF46x::GetScanPtr() const
{
...
void FwUpdaterCommF46x::WriteBinary(const char* buf, size_t n_bytes)
{
#ifdef _WIN32
...
void FwUpdaterCommF46x::WriteCmd(const std::string& cmd, bool wait_response)
{
...
void FwUpdaterCommF46x::WaitForStr(const char* wait_str, std::chrono::milliseconds timeout)
{
    ...
    {
        WaitForIdle();
        // Ignore preceding carriage return in wait_str.
        auto found = strstr(&_read_buffer[scan_idx], &wait_str[1]) != nullptr;
        if (found)
        {
        ...
        {
            ConsumeScanned();
            LOG_ERROR("FwUpdaterComm::WaitForStr", "Timeout waiting for \"%s\"", wait_str);
            throw std::runtime_error("FwUpdaterComm::WaitForStr failed");
        }
    }
}

void FwUpdaterCommF46x::StopReaderThread()
{
...
void FwUpdaterCommF46x::DumpSession(const char* filename)
{
