...
LicenseCheckStatus LicenseChecker::CheckLicense(const std::vector<unsigned char>& iv, const std::vector<unsigned char>& enc_session_token,
...
{
    std::vector<unsigned char> iv(data, data + AES_CTR_IV_SIZE_BYTES);
    std::vector<unsigned char> enc_session_token(data + AES_CTR_IV_SIZE_BYTES, data + AES_CTR_IV_SIZE_BYTES + LICENSE_SESSION_TOKEN_SIZE);
