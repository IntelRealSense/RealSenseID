...
LicenseResult LicenseUtils_nix::GetLicenseKey(std::string& license_key)
{
    ...
    if (file_exists(user_config_file)) {
        if (file_is_readable(user_config_file)) {
            ...
            return {LicenseResult::Status::Error,
                    "User license key file is present but process cannot read it. Check file permissions."};
        }
        ...
        if (file_is_readable(LICENSE_KEY_PATH_SYSTEM)) {
            ...
            return {LicenseResult::Status::Error,
                    "System license key file is present but process cannot read it. Check file permissions."};
        }
    } else {
        LOG_ERROR(LOG_TAG, "Looked in %s for user license.\n"
