...
inline std::vector<std::pair<std::string, std::string>> SupportedDevices()
{
    return {{"04d8", "00dd"}, {"2aad", "6373"}};
}

inline void ThrowIfFailed(const char* call, uvc_error_t status)
{
    ...
    _stream_attributes(stream_attributes), _camera_index(camera_number)
{
    ...
    if (_stream_attributes.format == MJPEG)
    {
        ...
        std::vector<int> fps_range {15, 14, 13}; // libuvc FPS range for MJPEG
